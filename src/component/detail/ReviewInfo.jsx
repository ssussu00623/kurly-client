...
export default function ReviewInfo({src, name, pid, setReviewCount}) {
    ...
    useEffect(() => {
        ...
        axios.post('http://43.201.27.254:9000/review/getList',{'pid':pid})
                .then(res => {
                    setData(res.data);
        ...
        axios.post('http://43.201.27.254:9000/review/getImages',{'pid':pid})
                .then(res => {
                    let newList = [];
    ...
    const orderByDate = () => {       
        axios.post('http://43.201.27.254:9000/review/getDateList',{'pid':pid})
    ...
    const increment = async (rid) => {
        const result = await axios.post('http://43.201.27.254:9000/review/getPlusCount',{'rid':rid})
        if(result.data.result_rows === 1) reloadData();
    }
    const decrement = async (rid) => {
        const result = await axios.post('http://43.201.27.254:9000/review/getMinusCount',{'rid':rid})
        if(result.data.result_rows === 1) reloadData();
    }
