...
export default function Find() {
    ...
    const handleFindIdSubmit = (e) => {
        ...
        if (!name) {
            ...
            // Axios 요청 - 아이디 찾기
            axios
                .post('http://43.201.27.254:9000/member/findid', { name, phone })
                .then((response) => {
    ...
    const handleFindPwdSubmit = (e) => {
        ...
        if (result) {
            // Axios 요청 - 비밀번호 찾기
            axios
                .post('http://43.201.27.254:9000/member/findpwd', { id, phone, emailname, emaildomain }) // emailname과 emaildomain을 합쳐서 서버에 전달
                .then((response) => {
                    if (response.data.success) {
