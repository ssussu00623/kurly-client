...
export default function Login() {
    ...
    const handleLoginSubmit = async (e) => {
        ...
        if (validateLogin(refs)) { 
            //서버전송
            try {
                const res = await axios.post('http://13.209.41.189:9000/member/login', formData);

                if (res.data.result_rows === 1) {
                    const user_type = await axios.post('http://13.209.41.189:9000/member/type', { 'id': formData.id });
